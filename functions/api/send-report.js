export async function onRequest(context) {
  ...
  if (request.method === "POST") {
    try {
      ...
      const { user_email, language, market_data, test_mode, market } = data;
      
      const RESEND_API_KEY = env.RESEND_API_KEY;
      if (!RESEND_API_KEY) throw new Error("RESEND_API_KEY is missing in environment variables.");
      ...
      if (test_mode) {
        ...
        if (!market_data) throw new Error("market_data is missing.");

        const isUS = market === 'us';
        const marketName = isUS ? "S&P 500" : "KOSPI";
        
        subject = language === 'ko' 
          ? `[anse.ai] ${marketName} 금융시장 심리 리포트 (${market_data.score || 0}점)` 
          : `[anse.ai] ${marketName} Market Sentiment Report (${market_data.score || 0} pts)`;
        ...
        // Indicators
        const indicatorsHtml = (market_data.indicators && market_data.indicators.length > 0)
        ...
        htmlContent = `
          <div style="font-family: 'Inter', -apple-system, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e7ecf4; border-radius: 16px; overflow: hidden; background-color: #ffffff;">
            ...
            <div style="padding: 40px 30px; text-align: center;">
              <div style="font-size: 60px; font-weight: 900; color: #3d84f5; margin-bottom: 10px;">${market_data.score || 0}</div>
            ...
            <a href="${(market_data.kospi && market_data.kospi.link) || '#'}" target="_blank" style="text-decoration: none; display: block;">
              <div style="padding: 20px 30px; background-color: #3d84f5; color: white; border-radius: 12px; margin: 0 30px 30px; text-align: left;">
                ...
                <div style="font-size: 28px; font-weight: 900;">${(market_data.kospi && market_data.kospi.value) || 'N/A'}</div>
                <div style="margin-top: 5px;">
                  ${(market_data.kospi && market_data.kospi.change_rate) ? `<span style="background: rgba(255,255,255,0.2); padding: 2px 8px; border-radius: 4px; font-weight: bold;">${market_data.kospi.change_rate}</span>` : ''}
                  <span style="margin-left: 5px; opacity: 0.9;">${(market_data.kospi && market_data.kospi.change_point) || ''}</span>
                </div>
              </div>
            </a>
      ...
      if (!resendRes.ok) throw new Error(`Resend error: ${resText}`);
