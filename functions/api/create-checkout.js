export async function onRequestPost(context) {
  ...
  try {
    ...
    // Polar API: Create Checkout Session
    // Ref: polar.txt -> post /v1/checkouts/
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: data.detail || "Failed to create checkout session" }), { status: response.status });
    }
